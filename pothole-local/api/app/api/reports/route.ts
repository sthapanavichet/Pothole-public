...
async function createReportFromFormData(request: Request) {
  ...
  const imageUrl = isLocalStorageMode()
    ? await uploadLocalImage(image, "original", request)
    : await uploadImage(image, "original");
  let annotatedImageUrl: string | null = null;

  if (annotatedImage instanceof File && annotatedImage.size > 0) {
    annotatedImageUrl = isLocalStorageMode()
      ? await uploadLocalImage(annotatedImage, "annotated", request)
      : await uploadImage(annotatedImage, "annotated");
  }
...
async function insertReport(report: CreateReportInput, request: Request) {
  ...
  if (isLocalStorageMode()) {
    const data = createLocalReport(withLocation);
...
async function existingCaptureResponse(
  ...
  if (isLocalStorageMode()) {
    const existing = findLocalReportByCaptureId(captureId);
    return existing
